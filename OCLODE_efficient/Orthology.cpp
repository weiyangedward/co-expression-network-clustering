                     ...
                     int num_species) 
{
    ...
    while (fgets(line, LINELEN, F))
    {
        ...
        string key1 = to_string(species.first) + "." + to_string(species.second) + "." + to_string(genes.first) + "." + to_string(genes.second);
        ...
        string key2 = to_string(species.second) + "." + to_string(species.first) + "." + to_string(genes.second) + "." + to_string(genes.first);
...
//            fprintf(stderr, "----- orth edge to spe2 gene108: spe %d, gene %d, line %s ------\n", species.second, stoi(species_network[species.second]->uniqId_to_geneName[genes.second]), line);
        ...
        spe_orth_node_adjList[species.first][genes.first].insert(to_string(species.second) + "." + to_string(genes.second));
        spe_orth_node_adjList[species.second][genes.second].insert(to_string(species.first) + "." + to_string(genes.first));
    }
    fclose(F);
...
//            for ( iter = o->weighted_orthEdge[spc1][spc2].begin(); iter != o->weighted_orthEdge[spc1][spc2].end(); ++iter)
            for (auto x : weighted_orthEdge[spe1][spe2])
            {
