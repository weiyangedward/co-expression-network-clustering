          ...
          has_noise_cluster(has_noise_cluster)
{
    ...
    show_log = false; // chanage manually
    ...
    if (coupling_constant > 0) /* has orthologous term */
    {
        current_orth_cost = get_orth_cost(orthology);
        fprintf(stderr, "orth_cost %g, coexpression cc = %g\n", current_orth_cost, 1-coupling_constant);
        current_total_cost = (1-coupling_constant) * current_coexpress_cost + coupling_constant * current_orth_cost;
    } else { /* no orthologous term */
        current_total_cost = current_coexpress_cost;
    }
...
void Clustering::Print(SpeciesNetwork **species_network)
{
    for (int spe=0; spe<num_species; spe++)
    {
        for (auto x : species_network[spe]->geneName_to_uniqId)
        {
            string name = x.first;
            int id = x.second;
...
void Clustering::learn_ground_state(SpeciesNetwork **species_network, Orthology *orthology)
{
    ...
    for(double temp=tempmax; temp >= tempmin; temp *= 0.9)
    {
        ...
        for (int i=0; i < 20*num_total_nodes; i++)
        {
            ...
            double new_coexpress_cost = old_coexpress_cost + coexpress_delta_cost;

            double new_total_cost = 0.0;
            /* compute new cost adding ortho term
             only compute get_orth_cost when needed */
            if (coupling_constant > 0)
            {
                double delta_orth_cost = get_orth_delta_cost(orthology, species_network, current_orth_cost);
                current_orth_cost += delta_orth_cost;
//                fprintf(stderr, "orth_cost %g, delta_orth_cost %g\n", current_orth_cost, delta_orth_cost);
//                new_total_cost = new_coexpress_cost + coupling_constant * get_orth_cost(orthology);

                new_total_cost = (1-coupling_constant) * new_coexpress_cost + coupling_constant * current_orth_cost;
            } else {
...
//                fprintf(stderr, "delta cost %g, old cost %g\n", delta_total_cost, old_total_cost);
            ...
            else
            {
                ...
                if (current_total_cost < best_total_cost - 1)
...
//    {
//        for (int node = 0; node < species_num_nodes[spe]; node++)
//            best_node_assigned_cluster[spe][node] = node_assigned_cluster[spe][node];
//    }

    for (auto x : changed_nodes)
    {
...
//        {
//            fprintf(stderr, "spe %d, cluster %d, size %d\n", i, j, species_cluster_size[i][j]);
//        }
//    }
...
double Clustering::get_cluster_coexpress_delta_cost_noNoiseCluster(int spe, int cluster, int perturb_node, SpeciesNetwork **species_network, bool is_new_state)
{
    ...
    for (auto node : species_network[spe]->node_adjList[perturb_node])
    {
    ...
    /* compute score */
    double new_in_edge, new_out_edge, new_inDens, new_outDens, new_score;
    if (after_changed_cluster_size >= 1) // cluster is not empty, then count in, out edges
    {
        ...
        {
            new_inDens = new_in_edge / possible_in_edge;
            new_outDens = new_out_edge / possible_out_edge;
            new_score = after_changed_cluster_size * new_inDens / (new_inDens + new_outDens);
        } else {
            new_in_edge = 0.0;
            new_inDens = 0.0;
            new_outDens = 0.0;
...
double Clustering::get_spe_score(int spe, SpeciesNetwork **species_network)
{
    ...
    for (int clus = 1; clus < num_clusters; clus++)
    {
        ...
        for (auto node : spe_cluster_nodes[spe][clus])
        {
            for (auto adj_gene : species_network[spe]->node_adjList[(node)])
            {
...
double Clustering::get_coexpress_cost(SpeciesNetwork **species_network)
{
    ...
    for (int spe = 0; spe < num_species; spe++)
    {
        unordered_map<int, int> total_cluster_outEdges; // #out-cluster edges
        unordered_map<int, int> total_cluster_edge; // #in-cluster edges
        map<int, int> total_cluster_nodes; // #nodes in cluster, ordered
        ...
        for (int i=0; i<species_num_nodes[spe]; i++)
        {
            ...
            {
                total_cluster_edge[node_assigned_cluster[spe][i]] += 0; // initialize in-cluster total edge = 0
                // increment #nodes in clus
                total_cluster_nodes[node_assigned_cluster[spe][i]] ++;
                /* loop through node_j in a spe, start from j=0
                 so that the out_edge can be double counted, which is needed */
                for (int j=0; j<species_num_nodes[spe]; j++)
                {
                    // skip noise cluster
                    if (node_assigned_cluster[spe][j] != 0)
                    {
                        ...
                        if (node_assigned_cluster[spe][i] == node_assigned_cluster[spe][j])
                        {
                            ...
                            if (species_network[spe]->is_edge(i, j))
                            {
                                // increment #in-cluster edges
                                total_cluster_edge[node_assigned_cluster[spe][i]]++;
                            }
                        } else { // i and j are in diff cluster
                            if (species_network[spe]->is_edge(i,j))
                            {
                                // increment #out-cluster edges
                                total_cluster_outEdges[node_assigned_cluster[spe][i]] ++;
                            }
                        }
                    }
                }
            } else { // if cur cluster is noise cluster
        ...
        // init cost of spc
        double spe_score = 0.0;
        for (auto x : total_cluster_nodes)
        {
            int cluster = x.first, cluster_size =x.second;
            // #in-cluster edges > 0, do not need to worry about 'noise cluster' since it does not take into account above
            int possible_in_edge = 0, possible_out_edge  = 0;
            double total_cluster_inDensity = 0.0, total_cluster_outDensity = 0.0;
            if (total_cluster_edge[cluster] > 0)
            {
                // #possible in-cluster edges
                possible_in_edge = cluster_size * (cluster_size-1) / 2;
                // #possible out-cluster edges
                possible_out_edge = ( species_num_nodes[spe] - noise_clus_size - (cluster_size) ) * (cluster_size);
                // in-density
                total_cluster_inDensity = double(total_cluster_edge[cluster]/2) / double(possible_in_edge);
                // out-density
                total_cluster_outDensity = double(total_cluster_outEdges[cluster]) / double(possible_out_edge);
                // cost of a spc = in-density * size / (in-density + out-density)
                species_cluster_coexpress_cost[spe][cluster] = (total_cluster_inDensity / (total_cluster_inDensity + total_cluster_outDensity)) * double(cluster_size);
...
//                fprintf(stderr, "cluster: %d, size: %d, in_edge: %d, out_edge: %d, possible_in_edge: %d\n", cluster, cluster_size, total_cluster_edge[cluster]/2, total_cluster_outEdges[cluster], possible_in_edge);
            ...
            species_cluster_inEdges[spe][cluster] = total_cluster_edge[cluster]/2;
            species_cluster_outEdges[spe][cluster] = total_cluster_outEdges[cluster];
...
//    fprintf(stderr, "===== get_orth_delta_cost spe %d, node %s ======\n", spe, (species_network[spe]->uniqId_to_geneName[perturb_node]).c_str());
    for (auto x : orthology->spe_orth_node_adjList[spe][perturb_node])
    {
...
//        total_noise_nodes += spe_cluster_nodes[spc1][0].size(); // sum up noise nodes
        ...
        for (int spe2=spe1+1; spe2 < num_species; spe2++) // avoid double-counting ortholog edges
        {
            /*=============
             ...
             ==============*/
            for (auto x : orthology->orth_edges[spe1][spe2])
            {
...
void Clustering::undo_perturb(int new_state)
{
    for (int i = undo_log_size; i>0; i--)
    {
        ...
        changed_nodes[to_string(spe) + "." + to_string(node)] = old_state;
...
//            if (newstate == oldstate or min_clus_size <= species_num_nodes[spc]/100) continue;
            ...
            changed_nodes[to_string(spc) + "." + to_string(node)] = newstate;
